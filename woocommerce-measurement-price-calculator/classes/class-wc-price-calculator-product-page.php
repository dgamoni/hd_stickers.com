...
class WC_Price_Calculator_Product_Page {
	...
	public function __construct() {
		...
		// Fix sale flash on pricing rules products - WC 2.3+ only
	...
	private function add_price_html_filters() {
		...
		// variation price adjustments for WC 2.4+
		...
		// variation price adjustments for WC 2.3
		remove_filter( 'woocommerce_get_variation_regular_price',    array( $this, 'get_variation_regular_price' ), 10 );
		remove_filter( 'woocommerce_get_variation_sale_price',       array( $this, 'get_variation_sale_price' ), 10 );
		remove_filter( 'woocommerce_get_variation_price',            array( $this, 'get_variation_price' ), 10 );
	}
	...
	/**
	 * Filter the regular price per unit min/max for a variable measurement price
	 * calculator product in WC 2.3 (the method using this filter is not used
	 * in WC core 2.4+)
	 *
	 * @since 3.3.1
	...
	public function get_variation_regular_price( $price, $product, $min_or_max, $display ) {
		...
		if ( $display ) {
			...
			$price = round( $product->get_display_price( $product->$variation_regular_price ), 2 );
		}
	...
	/**
	 * Filter the sale price per unit min/max for a variable measurement price
	 * calculator product in WC 2.3 (the method using this filter is not used
	 * in WC core 2.4+)
	 *
	 * @since 3.3.1
	...
	public function get_variation_sale_price( $price, $product, $min_or_max, $display ) {
		...
		if ( $display ) {
			...
			$price = round( $product->get_display_price( $product->$variation_sale_price ), 2 );
		}
	...
	/**
	 * Filter the price per unit min/max for a variable measurement price
	 * calculator product.
	 *
	 * @since 3.3.1
	...
	public function get_variation_price( $price, $product, $min_or_max, $display ) {
		...
		if ( $display ) {
			...
			$price = round( $product->get_display_price( $product->$variation_price ), 2 );
		}
	...
	public function price_per_unit_html( $price_html, $product ) {
		...
		$_product = $product;
		if ( isset( $product->variation_id ) && $product->variation_id ) { $_product = wc_get_product( $product->id ); }
